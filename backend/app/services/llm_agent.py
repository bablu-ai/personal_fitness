...
def chat(message: str, db: Session, user_id: str = "default") -> str:
    ...
    try:
        ...
        system_prompt = (
            ...
            "- **Bold** key terms, numbers, and action items.\n"
            "- Use bullet lists (- item) for 3+ parallel items or options.\n"
            "- Use numbered lists (1. step) for ordered steps or instructions.\n"
            "- Use ## section headers only when the reply has 2+ distinct sections.\n"
            "- Use a > blockquote for a single key tip or highlight.\n"
            "- Lead with the direct answer — context and detail follow.\n"
            "- Don't pad. Stop when answered.\n"
            "- No tables, no code blocks, no nested lists.\n\n"
            f"--- TODAY'S PROGRESS ---\n{today_context}"
            f"{plan_section}"
        )
